    ...
    def emit_alert(self, state: BehaviorState) -> BehaviorState:
        ...
        if (
            ...
            alert_data = {
                ...
                "action": state.get("action", ""),
                "location": state.get("location", ""),
                "clicks_per_minute": f"{state.get('clicks_per_minute', 0):.1f}",
                "is_sensitive_route": state.get("is_sensitive_route", False),
                "is_bot_like": state.get("is_bot_like", False),
                "is_odd_hour": state.get("is_odd_hour", False),
                "impossible_travel": state.get("impossible_travel", False),
                "deviates_from_baseline": state.get("deviates_from_baseline", False),
